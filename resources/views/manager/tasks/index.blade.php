...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <div class="buttons">
                <a href="{{ route('manager.task.create') }}" class="btn btn-primary">{{ trans('app.create') }}
                    {{ trans('app.task') }}</a>
            </div>
            <div class="card">
                                    ...
                                    <tr class="tr-shadow" id="row-{{ $emp->id }}">
                                        ...
                                        <td>
                                            <div class="table-data-feature">
                                                ...
                                                <a class="item" href="{{ route('manager.task.edit', $emp->id) }}"
                                                    data-toggle="tooltip" data-placement="top" title="Edit">
                                                    <i class="zmdi zmdi-edit"></i>
                                                </a>
                                                <button class="item delete-item" data-toggle="tooltip" data-placement="top"
                                                    data-url="{{ route('manager.task.destroy', $emp->id) }}"
    ...
    <script>
        $(function() {
            ...
            $('table').on('change', '.task-status', function() {
                var tag = $(this);
                $.ajax({
                    url: "{{ route('manager.task.updateStatus') }}",
                    method: "POST",
                    headers: {
